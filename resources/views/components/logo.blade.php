...
<div {{ $attributes->merge(['class' => 'flex flex-col items-center gap-1 ' . ($light ? 'text-white' : 'text-brand-blue')]) }}>
    ...
    @endif
    <div class="max-w-[120px] text-center font-serif text-[10px] leading-[1.4]">
        <div>{{ strtoupper(setting('clinic_name', 'Cringila General Medical Practice')) }}</div>
        <div class="mt-1 font-sans text-[6.5px] tracking-[.24em] text-brand-green">{{ strtoupper(setting('tagline', 'Healthcare for Every Generation')) }}</div>
    </div>
</div>
