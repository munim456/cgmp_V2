...
<div {{ $attributes->merge(['class' => 'inline-flex flex-col items-center text-center ' . ($light ? 'text-white' : 'text-brand-blue')]) }}>
    ...
    @else
        <div class="relative h-6 w-full overflow-hidden">
    ...
    @endif
    <div class="mt-1 whitespace-nowrap font-serif text-[9px] font-bold uppercase tracking-wide">{{ setting('clinic_name', 'Cringila General Medical Practice') }}</div>
    <div class="mt-0.5 whitespace-nowrap font-sans text-[5px] font-semibold tracking-[.25em] text-brand-green">{{ strtoupper(setting('tagline', 'Healthcare for Every Generation')) }}</div>
</div>
