...
<article class="rounded-2xl bg-white shadow-lg shadow-[#062238]/10 overflow-hidden flex flex-col" data-reveal>
    <a href="{{ route('blog.show', $post) }}" class="block aspect-[2/1] overflow-hidden bg-brand-blue-tint sm:aspect-[16/9]">
    ...
    <div class="flex flex-1 flex-col gap-1.5 p-3 sm:gap-2 sm:p-4">
        <div class="flex flex-wrap items-center gap-2 text-[11px] text-[#60758d] sm:text-xs">
            @if($post->category)
                <span class="rounded-full bg-brand-blue-tint px-2.5 py-0.5 text-[10px] font-semibold text-[#062238] sm:text-[11px]">{{ $post->category->name }}</span>
            @endif
