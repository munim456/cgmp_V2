...
<div class="mx-auto text-center {{ $nowrap ? 'max-w-6xl' : 'max-w-4xl' }}">
    ...
    <h2 @if($titleStyle) style="{{ $titleStyle }}" @endif class="mt-6 font-serif font-bold text-[#062238] {{ $titleClass ?? ($nowrap ? 'text-2xl sm:whitespace-nowrap sm:text-3xl md:text-4xl lg:text-5xl' : 'text-4xl md:text-6xl') }}">{{ $title }}</h2>
    @if($copy)
