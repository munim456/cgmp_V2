...
<div class="flex flex-col rounded-[18px] bg-white p-5 shadow-lg shadow-[#062238]/10 hover:-translate-y-1 transition-transform" data-reveal>
    <a href="{{ route('services.show', $service) }}" class="block flex-1">
        ...
        <h3 style="{{ text_style($service->text_styles, 'title') }}" class="font-serif text-lg font-bold">{{ $service->title }}</h3>
        <p style="{{ text_style($service->text_styles, 'short_description') }}" class="mt-2 text-sm leading-6 text-[#60758d]">{{ $service->short_description }}</p>
