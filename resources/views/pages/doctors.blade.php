...
<section class="bg-white px-6 py-24">
    ...
    <div class="mx-auto mt-14 grid max-w-6xl gap-8 md:grid-cols-2 lg:grid-cols-3">
        @foreach($doctors as $doctor)
            <div class="flex flex-col rounded-2xl bg-white p-7 shadow-[0_4px_24px_rgba(27,114,181,0.08)] transition-all duration-300 ease-out hover:-translate-y-1">
                <div class="mb-5 flex items-start gap-4">
                    ...
                    <div>
                        <h3 class="text-base font-bold text-[#08253C]">{{ $doctor->name }}</h3>
                ...
                @if($doctor->languageList())
                    <div class="mb-4 flex flex-wrap gap-2">
                        @foreach($doctor->languageList() as $language)
                            <span class="flex items-center gap-1 rounded-full bg-purple-50 px-3 py-1 text-xs font-medium text-purple-700">
                ...
                <div class="mb-4">
                    <p class="mb-2 flex items-center gap-1.5 text-xs text-gray-400">
                    ...
                    <div class="flex gap-1.5">
                        ...
                        @foreach(['mon' => 'M', 'tue' => 'T', 'wed' => 'W', 'thu' => 'T', 'fri' => 'F', 'sat' => 'S'] as $value => $label)
                            <span
                                class="flex h-7 w-7 items-center justify-center rounded-lg text-xs font-bold {{ in_array($value, $activeDays) ? 'bg-[#56A135] text-white' : 'bg-gray-100 text-gray-300' }}"
                                title="{{ ucfirst($value) }}"
                            >{{ $label }}</span>
                        @endforeach
                    </div>
                ...
                <div class="mt-auto">
                    <a href="{{ route('booking') }}" class="flex w-full items-center justify-center gap-2 rounded-xl bg-[#56A135] px-8 py-2.5 text-sm font-semibold text-white shadow-lg transition-all hover:bg-brand-green-dark">
                    ...
                    <a href="{{ route('doctors') }}" class="mt-1.5 block py-1.5 text-center text-sm font-medium text-[#114A75] hover:text-[#56A135]">
