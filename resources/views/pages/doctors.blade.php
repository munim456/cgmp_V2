...
<section class="bg-white px-6 py-24">
    ...
    >
        <div class="relative">
            <svg xmlns="http://www.w3.org/2000/svg" class="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <input
                type="text"
                x-model="search"
                placeholder="Search by doctor name..."
                aria-label="Search by doctor name"
                class="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm text-[#062238] placeholder:text-gray-400 focus:border-brand-blue focus:outline-none focus:ring-2 focus:ring-brand-blue/20"
            >
        </div>
        ...
        <div class="mt-3 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" role="group" aria-label="Filter doctors by specialty">
            @foreach($doctorRoles as $roleOption)
                <button
                    ...
                    :class="filter === @js($roleOption) ? 'bg-[#0F2A43] text-white' : 'border border-slate-200 bg-white text-gray-600 hover:border-brand-blue hover:text-brand-blue'"
        ...
        <p class="mt-10 text-center text-sm text-gray-500" x-show="visibleCount === 0">
