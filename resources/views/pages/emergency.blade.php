...
<section class="bg-gradient-to-b from-red-800 to-red-900 px-6 py-14 text-center text-white sm:py-20">
    ...
    <a href="tel:000" class="btn-lift mt-6 inline-flex items-center gap-3 rounded-2xl bg-white px-5 py-2.5 text-sm font-bold text-red-700 shadow-lg hover:bg-red-50 sm:mt-8 sm:px-7 sm:py-4 sm:text-base">
...
<section class="px-6 py-10 sm:py-16">
    <div class="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-[#e7edf3] shadow-sm">
        ...
        <div class="grid gap-3 p-4 sm:grid-cols-2 sm:gap-4 sm:p-6">
            ...
            @endforeach
            <div class="flex items-center justify-between gap-3 rounded-xl border border-brand-blue-tint bg-brand-blue-tint px-3 py-2.5 text-brand-blue sm:px-4 sm:py-3">
    ...
    <div class="mx-auto mt-6 max-w-4xl rounded-2xl border border-[#e7edf3] p-4 shadow-sm sm:mt-8 sm:p-8">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-serif text-base font-bold text-[#062238] sm:text-lg">After-Hours Care</h2>
        </div>
        <p class="mt-2 text-sm text-[#45627d] sm:mt-3 sm:text-base">When our clinic is closed and you need medical attention that is not life-threatening, you have several options:</p>
        <div class="mt-4 grid gap-2.5 sm:mt-5 sm:gap-3">
            @foreach([
                ...
                <div class="flex items-start gap-3 rounded-xl bg-brand-green-tint px-3 py-2.5 sm:px-4 sm:py-3">
                    ...
                    <span>
                        <span class="block text-sm font-bold text-[#062238] sm:text-base">{{ $title }}</span>
                        <span class="text-xs text-[#45627d] sm:text-sm">{{ $body }}</span>
                    </span>
                </div>
            @endforeach
        </div>
    ...
    <div class="mx-auto mt-6 max-w-4xl rounded-2xl border border-[#e7edf3] p-4 shadow-sm sm:mt-8 sm:p-8">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-serif text-base font-bold text-[#062238] sm:text-lg">Nearest Hospitals</h2>
        </div>
        <div class="mt-4 grid gap-2.5 sm:mt-5 sm:gap-3">
            @forelse($hospitals as $hospital)
                <div class="rounded-xl bg-brand-green-tint px-3 py-2.5 sm:px-4 sm:py-3">
                    <div class="flex items-center justify-between gap-3">
                        <span class="text-sm font-bold text-[#062238] sm:text-base">{{ $hospital['name'] }}</span>
                    ...
                    @if(!empty($hospital['address']))
                        <p class="mt-1 text-xs text-[#45627d] sm:text-sm">{{ $hospital['address'] }}</p>
                    @endif
